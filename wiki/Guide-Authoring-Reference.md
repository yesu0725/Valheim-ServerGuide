...
| `dvergr_recruited` | `raven` | Player frees a Dvergr (requires Lost Scrolls II); optional `caste` filter |
| `dvergr_duel_won` | `message` | Player wins a Dvergr duel (requires Lost Scrolls II); `caste` = winner |
| `dvergr_level_up` | `message` | A recruited Dvergr levels up (requires Lost Scrolls II); optional `caste` + `level` |
